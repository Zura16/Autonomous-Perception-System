...
| C-30 | "Measured the full pipeline at p99 61.9 ms against a 103.6 ms sensor budget — 60% of it — and declined to optimize anything, because no stage misses its share." | [Row 8.1](benchmarks.md) | `7642fab` | Apple M2 MPS, YOLOv8n 640 px, batch 1, N=1548 val frames, 30 warmup excluded, device-synchronized. Detection and lanes are 63% of it; every geometry stage combined is 1.1%. Image decode excluded as sensor-side work. |
| C-31 | "Showed that summing per-stage p99 latencies overstates the true pipeline p99 by 7%, and that a 300-frame sample understated it by 37% — so I report a measured total over the full split." | [Row 8.2, 8.4](benchmarks.md) | `7642fab` | Percentiles are not additive because stages do not peak on the same frames. The direction of the summing error is not fixed; correlated stages would make it understate instead. |
| C-32 | "Found the over-budget frame count is not reproducible run to run (3 vs 7 of 1548 on identical configs) and reported it as a range, because those frames are host scheduling stalls rather than pipeline work." | [Row 8.3](benchmarks.md) | `7642fab` | p99 itself is stable to ±4%. The stage that spikes changes between runs, which is how the stalls are identified as host-side. General-purpose OS, no real-time scheduling. |
