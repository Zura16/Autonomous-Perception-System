...
| C-36 | "Pinned the exact torch and ultralytics versions every benchmark row was measured on, after finding unbounded dependency specifiers meant a routine upgrade could silently move every published number with no commit to blame." | [Row 0.2](benchmarks.md) | *pending* | `requirements-lock.txt` plus upper bounds in `pyproject.toml`. ultralytics ships the weights and NMS defaults, so a minor release moves AP and everything derived from those boxes. |
