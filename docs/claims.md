...
| C-2 | "Found that ground truth is undefined for occluded objects — a naive LiDAR-in-box estimator reports the occluder at 11.9 m MAE — so only 38% of labelled objects receive ground truth and every range figure is a lower bound." | [Row 1.7, 1.2](benchmarks.md) | `b1f5eb7` | val split. |
...
| C-30 | "Measured the full pipeline at p99 59.7 ms against a 103.6 ms sensor budget — 58% of it — and declined to optimize anything, because no stage misses its share." | [Row 8.1](benchmarks.md) | `7642fab` | Apple M2 MPS, YOLOv8n 640 px, batch 1, N=1548 val frames, 30 warmup excluded, device-synchronized. Detection and lanes are 63% of it; every geometry stage combined is 1.1%. Image decode excluded as sensor-side work. |
| C-31 | "Showed that summing per-stage p99 latencies overstates the true pipeline p99 by 8% while understating a correlated subset by 5%, and that a 300-frame sample understated it by a third — so I report a measured total over the full split." | [Row 8.2, 8.4](benchmarks.md) | `7642fab` | Percentiles are not additive because stages do not peak on the same frames. The direction of the summing error is not fixed; correlated stages would make it understate instead. |
| C-32 | "Found the over-budget frame count is not reproducible run to run (3, 7 and 3 of 1548 across three runs) and reported it as a range, because those frames are host scheduling stalls rather than pipeline work." | [Row 8.3](benchmarks.md) | `7642fab` | p99 itself is stable to ±4%. The stage that spikes changes between runs, which is how the stalls are identified as host-side. General-purpose OS, no real-time scheduling. |
| C-33 | "Closed a near-field error source that had been open for six phases by deriving it from geometry: below 5.91 m the ground contact point falls outside the image, so monocular range saturates — predicted with no free parameters and measured at 6.03 m on validation and 6.02 m on a held-out split." | [Row 9.8](benchmarks.md) | `facbe75` | 100% of sub-5.91 m objects are over-estimates on both splits. It is a field-of-view limit of a 1.655 m camera mount, not an estimator defect — a production stack covers that band with a wider lens, a lower mount, or radar. |
| C-34 | "Showed the size-prior estimator fails in the same band and slightly worse, despite containing no ground plane, because the image edge truncates box height as well as hiding the contact point." | [Row 9.8](benchmarks.md) | `facbe75` | val 58.8% MAPE vs contact-point's 49.2% below 5.91 m. I expected size-prior to be immune and it is not; one boundary produces two distinct failure mechanisms. |
| C-35 | "Fixed it with a clip margin swept on validation alone, cutting near-field error 21.0 to 12.4% with every other range bin untouched — and reported that the fix transfers to held-out data far less well than the prediction did." | [Row 9.9, 9.10](benchmarks.md) | `facbe75` | test gains only 2.2 points (22.5 to 20.3%) because 17% of affected boxes leak past a 10 px margin and the survivors are worse there (19 objects, 82% MAPE, 99% of the bin's bias). Margin deliberately NOT retuned on test. Coverage cost: near-field validity falls to 68% val / 52% test. |
| C-36 | "Pinned the exact torch and ultralytics versions every benchmark row was measured on, after finding unbounded dependency specifiers meant a routine upgrade could silently move every published number with no commit to blame." | [Row 0.2](benchmarks.md) | `629ba88` | `requirements-lock.txt` plus upper bounds in `pyproject.toml`. ultralytics ships the weights and NMS defaults, so a minor release moves AP and everything derived from those boxes. |
